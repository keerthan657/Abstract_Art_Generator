...
public class canvasBox
{
    ...
    public canvasBox(layer layer1, layer layer2, colorPalette myColorPalette)
    {
        this.layer1 = layer1;
        this.layer2 = layer2;
    ...
    {
        drawable myDrawable = new drawable();
        myDrawable.setColorPalette(myColorPalette);

        Group layout = myDrawable.drawShape(layer1, layer2);

        //Color[] bgColors = new Color[]{Color.CADETBLUE, Color.BURLYWOOD, Color.CORAL, Color.DARKORCHID, Color.DEEPSKYBLUE};
        //Scene scene = new Scene(layout, 800, 600, bgColors[(int)(Math.random() * bgColors.length)]);
    ...
    public void fillColors(ArrayList<Color> al, int num)
    {
        for(int i=0; i<num; i++) {
            Color c = drawable.myColors[(int) (Math.random() * drawable.myColors.length)];
            if(c!=Color.BLACK)
                c = Color.WHITE;
    ...
    public boolean isPresent(Color c, ArrayList<Color> al)
    {
        for(Color x : al)
        {
            if(x==c)
